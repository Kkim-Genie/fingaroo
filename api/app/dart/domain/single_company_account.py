...
class DartSingleCompanyAccountItem(BaseModel):
...
class DartSingleCompanyAccount(BaseModel):
    status: str # 000 정상, 013 데이터 없음
    message: str # 메시지
    list: list[DartSingleCompanyAccountItem]

def get_single_company_account_description():
    return """
    단일회사 계정

    이 API는 개별 기업의 재무제표 계정 정보를 조회할 수 있는 기능을 제공합니다.
